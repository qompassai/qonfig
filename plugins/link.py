# /qompassai/qonfig/config/plugins/link.py
# Qompass AI Qonfig Link
    ...
    def _link(self, source, link_name, relative):
        ...
        # again, we use absolute_source to check for existence
            ...
            if self._is_link(link_name):
                self._log.warning("Nonexistent target %s -> %s" % (link_name, source))
            else:
                self._log.warning(
                    "Nonexistent target for %s : %s" % (link_name, source)
                )
        else:
